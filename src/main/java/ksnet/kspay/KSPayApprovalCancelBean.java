...
public class KSPayApprovalCancelBean{
    ...
    public String[]
                    ...
                    MCpId  					 = new String[MAXSIZE],    	 // 가맹점등록정보		
	...
	//휴대폰 M100 
  public boolean MobileDataMessage(
   String ApprovalType		,  /* 구분코드 */
   String TransactionNo	  ,  /* 거래번호 */
   String CommSele				,  /* 통신사구분 */
   String MobileNo				,  	/* 휴대폰번호 */
   String ApprAmount			,	/* 승인금액 */
   String CompSele				,  /* 업체코드 */
   String ServerInfo			,  /* 사용자고유key */
   String GoveTrno				,  /* 사용자고유key */
   ...
   String Filler )		 
	{
		...
		TmpSendMsg.append(this.format(ServerInfo			,      128, 'X'));
		TmpSendMsg.append(this.format(GoveTrno				,    		20, 'X'));
		TmpSendMsg.append(this.format(BypassMsg			  ,    	 100, 'X'));
		TmpSendMsg.append(this.format(Filler          ,      111, 'X'));

		this.SendMsg += TmpSendMsg.toString();
		this.SendCount++;
		System.out.println("MobileDataMessage=["+TmpSendMsg.toString()+"]");
		return true;
	}
	...
	public boolean ReceiveDataMessage(int iCnt) throws IOException
	{
		...
		for(int i=0; i < iCnt; i++)
		{
			...
			//핸드폰결제취소
			else if(this.ApprovalType[i].substring(0,3).equals("M11")) 	
			{
			...
			// 핸드폰 결제 M100
			else if(this.ApprovalType[i].substring(0,3).equals("M10"))
			{
				...
				this.MCpId            [i] = new String(this.KSPaySocket.read(40  ) ,MSG_ENCODING);      /* 업체등록코드 */
				this.MFiller          [i] = new String(this.KSPaySocket.read(66  ) ,MSG_ENCODING);      /* 예비 */
				...
				TmpReceiveMsg.append(this.MBypassMsg    [i]);
				TmpReceiveMsg.append(this.MCompCode     [i]);
				TmpReceiveMsg.append(this.MCommSele     [i]);
				TmpReceiveMsg.append(this.MMobileNo     [i]);
				TmpReceiveMsg.append(this.MApprAmt      [i]);
				TmpReceiveMsg.append(this.MCpId         [i]);			
				TmpReceiveMsg.append(this.MFiller       [i]);
